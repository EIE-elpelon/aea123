...
if(!persona) return message.channel.send(new Discord.MessageEmbed()
                                              .setDescription("<:Cancel:635072844782370828> | Debes mencionar a alguien para darle un rol")
                                              .setColor("RED"))  

let nombrerol = args.slice(1).join(' ')
if(!nombrerol) return message.channel.send(new Discord.MessageEmbed()
                                              .setDescription("<:Cancel:635072844782370828> | Debes mencionar un rol a agregar")
                                              .setColor("RED"))  
...
if(!rol){
  ...
  return message.channel.send(embed)
    ...
    let embed = new Discord.MessageEmbed()
  embed.setColor("RED")
  embed.setDescription("<:Cancel:635072844782370828> | Lo siento, pero no puedo darle ese rol a nadie, debido a que esta mas alto que mi rol.")
  ...
  return message.channel.send(embed)
    ...
    let embed = new Discord.MessageEmbed()
  embed.setColor("RED")
  embed.setDescription("<:Cancel:635072844782370828> | Ese rol es mas alto que tu rol mas alto (en lo que a jerarquia se refiere), asi no puedes darselo a nadie.")
...
persona.roles.add(rol.id).catch(e => message.reply(error))
  
  let si = new Discord.MessageEmbed()
  si.setColor("GREEN")
  si.setDescription(`<:Verified:635075337100853260> | Listo, le agrege el rol **${rol.name}** a **${persona.user.username}**`)
